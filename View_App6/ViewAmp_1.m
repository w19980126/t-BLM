...
delete(ROI)
Intensity = handles.Intensity;
FftFrom = handles.FftFrom;
FftTo = handles.FftTo;
if FftFrom == []
    FftFrom = 1;
elseif FftTo == [];
    FftTo = size(Intensity,3);
end
ROI_Mean = zeros(FftTo-FftFrom+1,1)';
for ii = 1:length(ROI_Mean)
    ROI_Mean(ii) = sum(sum(Mask.*Intensity(:,:,ii-1+FftFrom)))/sum(Mask(:));
end
axes(handles.axes4)
Fs = handles.SampleRate;
temp1 = abs(fft(ROI_Mean))/length(ROI_Mean);    % translate the amplitude after FFT into 
                                                % the amplitude of light intensity change
temp2 = temp1(1:floor(length(ROI_Mean)/2) - 1); % Nyquist sampleing theorem
...
axes(handles.axes3)
handles.hf3 = plot(FftFrom:FftTo,ROI_Mean,'k','linewidth',1);
...
if size(All_Raw_List,1) ~= 0
    ...
    if length(All_Raw_List) < Frame + handles.PreviewNum - 1
    ...
    if length(All_Tiff_List) < Frame + handles.PreviewNum-1
...
function FftFrom_Callback(hObject, eventdata, handles)

handles.FftFrom = str2num(get(hObject,'string'));
guidata(hObject,handles);

function FftFrom_CreateFcn(hObject, eventdata, handles)

if ispc && isequal(get(hObject,'BackgroundColor'), get(0,'defaultUicontrolBackgroundColor'))
    set(hObject,'BackgroundColor','white');
end

function FftTo_Callback(hObject, eventdata, handles)

handles.FftTo = str2num(get(hObject,'string'));
guidata(hObject,handles);

function FftTo_CreateFcn(hObject, eventdata, handles)
